...
/*   Updated: 2023/11/23 13:35:40 by qdenizar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_pf(t_cub *cub)
{
	...
	if (!cub->pf[0] || !cub->pf[1] || !cub->pf[2]
		...
		|| !cub->pf[8])
	{
		printf("Cub3D: Error: Load_pf Failed to load one or more the images\n");
		free_struct(cub);
		exit(1);
	}
}
...
void	image_manager(t_cub *cub)
{
	...
	{
		printf("Cub3D: Error: image_manager Failed to load one or more the images\n");
		free_struct(cub);
		exit(1);
	}
